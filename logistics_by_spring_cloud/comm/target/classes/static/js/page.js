...
function turnPage(postUrl, url, page) {
	pageUrl = encodeURI(encodeURI(url));
	pageNum = page;
				...
				document.getElementById(pageUtilForm).submit = function(){
					$.ajax({
						...
						success:function(data){
							document.getElementById(parentUrl).innerHTML = data;
						}
					});
					return false;
				};
...
//ajax翻页
	...
	function getContent(url,div){
		document.getElementById(div).innerHTML = "<center><img src='<%=basePath%>/images/ajax/ajax-loader.gif'><br>正在查询...</center>";
		var xmlHttp=createXMLHttpRequest();
